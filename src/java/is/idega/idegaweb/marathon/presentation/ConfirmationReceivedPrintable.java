...
public class ConfirmationReceivedPrintable extends Window {

	private boolean isIcelandicPersonalID = false;
	
	public ConfirmationReceivedPrintable() {
		setResizable(true);
		setHeight(450);
		setWidth(650);
	}
	...
	public void main(IWContext iwc) throws Exception {
		...
		String greeting = iwrb.getLocalizedString("lv_conf.hello_participant",
				"Dear participant");

		table.add(getHeader(greeting), 1, row++);
		table.setHeight(row++, 16);

		table.add(getText(iwrb.getLocalizedString("lv_conf.payment_received",
		...
		runnerTable.add(getHeader(iwrb.getLocalizedString(
				"lv_conf.runner_name", "Runner name")), 1, 1);
		runnerTable
				.add(getHeader(iwrb.getLocalizedString("lv_conf.runner_run",
						"Run")), 2, 1);
		runnerTable.add(getHeader(iwrb.getLocalizedString("lv_conf.shirt_size",
				"Shirt size")), 3, 1);
		...
		creditCardTable.add(
				getHeader(iwrb.getLocalizedString(
						"lv_conf.payment_received_timestamp",
						"Payment received")
						+ ":"), 1, 1);
		creditCardTable.add(getText(stamp.getLocaleDateAndTime(
				...
				2, 1);
		creditCardTable.add(
				getHeader(iwrb.getLocalizedString("lv_conf.card_number",
						"Card number") + ":"), 1, 2);
		creditCardTable.add(getText(cardNumber), 2, 2);
		creditCardTable.add(
				getHeader(iwrb.getLocalizedString("lv_conf.amount", "Amount")
						+ ":"), 1, 3);
		creditCardTable.add(
		...
		table.setHeight(row++, 16);
		table.add(getText(iwrb.getLocalizedString("lv_conf.best_regards",
				"Best regards,")), 1, row++);
